...
// 推广位真正在用 AdSlot（PromotionCard 是它的死重复副本，已删）。插槽里放一个
// "错误/刷新"块，验证 usePromotionCharge 通过插槽暴露的 error/unavailable 契约。
const renderCard = async () => {
  wrapper = mount({
    components: { AdSlot },
    data: () => ({ item: promotion }),
    template: `<AdSlot :item="item" @charged="onCharged">
      <template #default="{ clicking, error }">
        <span class="probe-clicking">{{ clicking }}</span>
        <span v-if="error" class="probe-error">{{ error }}</span>
        <span>查看推广商品 →</span>
      </template>
    </AdSlot>`,
    methods: { onCharged(item: Promotion) { (this as any).$emit('charged', item); } }
...
describe('商城推广与推荐的独立浏览器行为', () => {
  ...
  it('点击等待原曝光回执；双击只发送一次CPC，并使用服务端原回执进入商品详情', async () => {
    ...
    await card.find('button').trigger('click'); await card.find('button').trigger('click'); await flushPromises();
    expect(calls.some(call => call.path.endsWith('/clicks'))).toBe(false);
    ...
    expect(card.emitted('charged')).toEqual([[promotion]]);
  ...
  it.each(['exposure', 'click'])('投放暂停或版本变化在%s阶段拒绝时只显示刷新入口，不伪造跳转或新曝光', async (stage) => {
    ...
    const card = await renderCard(); observers[0]!.show(1); await flushPromises();
    if (stage === 'click') { await card.find('button').trigger('click'); await flushPromises(); }
    // 被拒后：插槽里能看到原因，按钮禁用（AdSlot 把 unavailable 用在 disabled 上），不会跳转
    expect(card.text()).toContain('可投状态已变化');
    expect(card.find('button').attributes('disabled')).toBeDefined();
    await card.find('button').trigger('click');
    expect(card.emitted('charged')).toBeUndefined();
  ...
  it('不确定点击结果只重试原幂等ID，切换身份后不继续进入旧商品', async () => {
    ...
    const card = await renderCard(); observers[0]!.show(1); await flushPromises();
    await card.find('button').trigger('click'); await flushPromises(); expect(card.emitted('charged')).toBeUndefined();
    await card.find('button').trigger('click'); await flushPromises();
    ...
    session.value!.actor.actor_id = 'v2'; await card.find('button').trigger('click'); await flushPromises();
    expect(calls.filter(call => call.path.endsWith('/ads/clicks'))).toHaveLength(2); expect(card.emitted('charged')).toHaveLength(1);
  });
