<template>
  <section class="page">
    ...
    <section v-if="detail" class="panel product-detail" aria-label="商品规格与下单">
      ...
      <p v-else-if="authStore.isTrial" class="muted">只读试用不能下单。请用演示买家 <code>shopper@smartlect.demo</code> 登录后再让导购下单。</p>
      ...
      <div class="actions-inline"><button class="primary" type="button" :disabled="busy || !canBuy" @click="buy">生成下单确认卡</button><button type="button" @click="consult">问这件</button></div>
...
async function buy() {
  if (authStore.isTrial) { error.value = '只读试用不能下单，请改用演示买家账号。'; return; }
