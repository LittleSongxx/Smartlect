...
set -uo pipefail
EV=/opt/cluster/evidence/loadtest-$(date +%Y%m%d-%H%M)-assistant
mkdir -p "$EV"
...
for VUS in 1 2 4 8; do
  ...
  ssh root@172.19.34.202 "k6 run --summary-trend-stats='avg,med,p(90),p(95),max' -e BASE=http://172.21.131.151 -e VUS=$VUS -e HOLD=90s /root/assistant-chat.js" > "$EV/chat-$VUS.log" 2>&1
...
for VUS_PER in 25 50 100 200; do
  ...
  ssh root@172.19.34.202 "k6 run --summary-trend-stats='avg,med,p(90),p(95),max' -e BASE=http://172.21.131.151 -e VUS=$VUS_PER -e HOLD=60s /root/assistant-control.js" > "$EV/ctl-$VUS_PER-node2.log" 2>&1 &
  P2=$!
  ssh root@172.19.34.203 "k6 run --summary-trend-stats='avg,med,p(90),p(95),max' -e BASE=http://172.21.131.151 -e VUS=$VUS_PER -e HOLD=60s /root/assistant-control.js" > "$EV/ctl-$VUS_PER-node3.log" 2>&1 &
  P3=$!
  wait $P2 $P3
