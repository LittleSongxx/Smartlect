...
| `growth-chat.js` | AI 对话全链路（session→会话→消息→轮询 run 终态，真 LLM） | `ORIGIN` 必须是后端白名单值（见踩坑） |
| `growth-control.js` | AI 控制面（session+推荐，无 LLM） | — |
| `loadtest-suite.sh` | browse 四档双发套件（node1 发起，证据落 /opt/cluster/evidence） | — |
| `loadtest-growth.sh` | chat 1/2/4/8 + control 25-200 双档套件 | — |
...
**踩坑**：① k6 打 growth 写接口时 `Origin` 头必须是 `SMARTLECT_ALLOWED_ORIGINS` 白名单值
