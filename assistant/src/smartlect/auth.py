...
class IdentityBridge:
    def __init__(self, config, *, transport=None, connect=None):
    ...
    def visitor_id(self, proof):
        from smartlect.guest_token import verify_visitor_jwt
        if proof.count(".") == 2:
            try:
                payload = verify_visitor_jwt(self.secret, proof)
            except ValueError:
                raise HTTPException(401, 'invalid_visitor') from None
    ...
    def csrf_token(self, actor):
        return self._sign("csrf", {"actor": actor.actor_id, "type": actor.subject_type,
                                  "session": actor.session_id, "scope": actor.execution_scope_id,
    ...
    def require_csrf(self, request, actor):
        ...
        if not consume(self.connect, jti, actor_id=actor.actor_id, session_id=actor.session_id):
            raise HTTPException(403, "csrf_replay")
